...
控制逻辑是处理器设计中最有挑战的部分：验证正确性最为困难，同时也最难进行时序优化。**例外** (exception) 和**中断** (interrupt) 是控制逻辑需要实现的人物之一。除分支指令外，它式另一种改变指令执行控制流的方式。
...
操作系统进行例外处理，除了引发例外的之另外，还必须获得例外发生的原因。第一种方法，也是 RISC-V 中使用的方法是设置**系统例外原因寄存器** (Supervisor Exception Cause Register, SCAUSE)，该寄存器中记录了例外原因。另一种方法是使用**向量式中断** (vectored interrupt)，该方法用基址寄存器加上例外原因（作为偏移）作为目标地址来完成控制流转换，从而让操作系统根据例外向量起始地址来确定例外原因。
