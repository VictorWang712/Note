...
对于递推关系 $a_{n} = c_{1} a_{n - 1} + c_{2} a_{n - 2}$，设方程 $r^{2} = c_{1} r + c_{2}$ 的两个根为 $r_{1}$ 和 $r_{2}$。若 $r_{1} \neq r_{2}$，则 $a_{n} = a_{1} r_{1}^{n} + a_{2} r_{2}^{n}$；若 $r_{1} = r_{2} = r_{0}$，则 $a_{n} = a_{1} r_{0}^{n} + a_{2} n r_{0}^{n}$。其中 $a_{1}$ 和 $a_{2}$ 可以根据初始条件解出。

这一定理的证明请读者参阅组合数学相关教科书或网络资料。

进一步地，我们考察更高阶的，也就是一般的情况。

若特征方程 $r^{k} = c_{1} r^{k - 1} + \cdots c_{k}$ 有 $k$ 个不相等的根 $r_{1}, r_{2}, \cdots r_{k}$，则 $a_{n} = a_{1} r_{1}^{n} + a_{2} r_{2}^{n} + \cdots a_{k} r_{k}^{n}$ 是递推关系 $a_{n} = c_{1} a_{n - 1} + \cdots c_{k} a_{n - k}$ 的解。其中 $a_{1}, a_{2}, \cdots a_{k}$ 可以根据初始条件解出。
...
$$\begin{aligned}
    a_{n} = & (a_{1, 0} + a_{1, 1} n + \cdots + a_{1, m_{1} - 1} n^{m_{1} - 1}) r_{1}^{n} \\
    & + (a_{2, 0} + a_{2, 1} n + \cdots + a_{2, m_{2} - 1} n^{m_{2} - 1}) r_{2}^{n} \\
    & + \cdots \\
    & + (a_{t, 0} + a_{t, 1} n + \cdots + a_{t, m_{t} - 1} n^{m_{t} - 1}) r_{t}^{n}
\end{aligned}
$$

是递推关系 $a_{n} = c_{1} a_{n - 1} + \cdots c_{k} a_{n - k}$ 的解。其中 $a_{i, j}$ 可以根据初始条件解出。
...
同时，为了后续的数学表示简便，我们还定义广义二项式系数。

对于实数 $u$ 和非负整数 $k$，广义二项式系数
